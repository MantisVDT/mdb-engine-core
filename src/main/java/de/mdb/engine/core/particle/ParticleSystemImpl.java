...
import de.mdb.engine.core.particle.emitter_shape.ParticleSphereShape;
import de.mdb.engine.core.util.Clock;

public class ParticleSystemImpl extends ParticleSystem{
    ...
    public ParticleSystemImpl(float pps, float speed, float gravityComplient, float lifeLength, float scale) {
    	super(new ParticleSphereShape());
        this.pps = pps;
    ...
    public void setDirection(Vector3f direction, float deviation) {
        ...
        float directionDeviation = (float) (deviation * Math.PI);
    }
    ...
    public void generateParticles(Vector3f systemCenter) {
        float particlesToCreate = pps * Clock.getDeltaTime();
...
//      velocity = generateRandomUnitVector();
        velocity = generateRandomUnitVectorWithinCone(direction, 0.5f);
        velocity.normalize();
